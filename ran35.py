    ...
    for root, dirs, files in os.walk(folder_path):
        for archive_filename in files:
            ...
            if archive_extension.lower() in ['.zip', '.gz', '.tar']:
                extract_folder = folder_path / 'archives' / normalized_filename.removesuffix(archive_extension)
                try:
                    patoolib.extract_archive(archive_path, outdir=extract_folder)
                    print(f"Extracted {archive_filename} to {extract_folder}")
                except Exception as e:
                    print(f"Failed to extract {archive_filename} in {folder_path}. Error: {e}")
